...
export const BathList: FC<PetShopListProps> = ({ petShops }) => {
    console.log(petShops);

    return (
        <div className="flex-1 overflow-y-auto p-2">
            {petShops ? petShops.map((item, index) => (
                <BathItem item={item} key={index} />
            )) : <>no results</>}
        </div>
    )
}

interface BathItemProps {
    item: {
        name: string;
        address: string;
        rating: number;
...
const BathItem: FC<BathItemProps> = ({ item }) => {
    const { name, address, rating, id } = item;
    
    return (
        <>
            <Link className="flex items-center p-2 bg-zinc-100 shadow-lg rounded-md border-slate-900 mb-4" href={`/bath/${id}`}>
